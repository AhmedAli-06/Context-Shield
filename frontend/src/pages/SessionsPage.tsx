...
import {
  Smartphone, Monitor, Globe, XCircle, Shield, RefreshCw,
  Search,
} from "lucide-react";
    ...
    <motion.div variants={stagger} initial="hidden" animate="visible">
      ...
      <motion.div className="card" variants={item}>
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {loading ? (
            ...
            <EmptyState icon={Shield} title="No sessions found" description="Sessions will appear here as users access the system." />
          ) : (
            <div>
              {activeSessions.length > 0 && (
                <div>
                  <div style={{ padding: "8px 16px", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.6px", color: "var(--accent-green)", fontWeight: 600, borderBottom: "1px solid var(--hairline)" }}>
              ...
              {inactiveSessions.length > 0 && (
                <div>
                  <div style={{ padding: "8px 16px", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.6px", color: "var(--stone)", fontWeight: 600, borderBottom: "1px solid var(--hairline)", borderTop: "1px solid var(--hairline)" }}>
      ...
      <Modal open={!!revokeId} onClose={() => setRevokeId(null)} title="Revoke Session">
        ...
        <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end", marginTop: "var(--space-xl)" }}>
          ...
          <button className="btn btn-primary" style={{ background: "var(--accent-red)", color: "white", borderColor: "var(--accent-red)" }} onClick={handleRevoke} disabled={revoking}>
    ...
    <div className="session-row">
      ...
      <div className="session-row-info">
        <div className="session-row-top">
          ...
          <span className={`badge ${session.is_active ? "allow" : "denied"}`} style={{ fontSize: "10px", padding: "2px 8px" }}>
        ...
        <div className="session-row-meta">
          ...
          {session.location && <><span className="meta-sep">·</span><span>{session.location}</span></>}
        ...
        <div className="session-row-time">
          ...
          {session.last_active_at && <> · Last active {new Date(session.last_active_at).toLocaleString()}</>}
        </div>
      </div>
      {session.is_active && (
        <button className="btn btn-ghost btn-sm" style={{ color: "var(--accent-red)" }} onClick={onRevoke}>
