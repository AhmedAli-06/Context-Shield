...
export default function AlertsPage() {
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible">
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                        ...
                        {alert.resolved_at && (
                          <span style={{ fontSize: "11px", color: "var(--accent-green)", marginLeft: "auto" }}>
                            Resolved
                          </span>
                        )}
                        {alert.acknowledged_at && !alert.resolved_at && (
                          <span style={{ fontSize: "11px", color: "var(--accent-blue)", marginLeft: "auto" }}>
                            Acknowledged
                          </span>
                        )}
                      </div>
                      <p style={{ fontSize: "13px", fontWeight: 500, color: "var(--charcoal)", marginBottom: "2px" }}>
                        {alert.title}
                      </p>
                      {alert.description && (
                      ...
                      <div style={{ display: "flex", gap: "16px", marginTop: "8px", fontSize: "11px", color: "var(--ash)" }}>
                        ...
                        <>
                          {!alert.acknowledged_at && (
                            <button
                              ...
                              title="Acknowledge"
                            >
                              {isProcessing ? <Loader2 size={13} className="spinning" /> : <Eye size={13} />}
                              Ack
                            </button>
                          )}
                          <button
      ...
      <Modal open={!!resolveTarget} onClose={() => { setResolveTarget(null); setResolveNotes(""); }} title="Resolve Alert">
        ...
        <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end", marginTop: "var(--space-xl)" }}>
          <button className="btn btn-ghost" onClick={() => { setResolveTarget(null); setResolveNotes(""); }}>Cancel</button>
          <button className="btn btn-primary" style={{ background: "var(--accent-green)", color: "#000", borderColor: "var(--accent-green)" }} onClick={handleResolve} disabled={actionLoading === resolveTarget}>
      ...
      <Modal open={!!detailAlert} onClose={() => setDetailAlert(null)} title="Alert Details">
        {detailLoading ? (
          ...
          <div>
            <div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "var(--space-lg)" }}>
            ...
            <h4 style={{ fontSize: "15px", fontWeight: 500, marginBottom: "var(--space-sm)", color: "var(--ink)" }}>{detailAlert.title}</h4>
            {detailAlert.description && (
              <p style={{ fontSize: "13px", color: "var(--ash)", marginBottom: "var(--space-xl)" }}>{detailAlert.description}</p>
            )}
            <div className="stat-row">
              <div>
                <span className="stat-row-label">Triggered</span>
                <span className="stat-row-value">{new Date(detailAlert.triggered_at).toLocaleString()}</span>
              </div>
              {detailAlert.trust_score_at_trigger !== undefined && (
                <div>
                  <span className="stat-row-label">Trust Score</span>
                  <span className="stat-row-value">{(detailAlert.trust_score_at_trigger * 100).toFixed(0)}</span>
                </div>
              )}
              {detailAlert.acknowledged_at && (
                <div>
                  <span className="stat-row-label">Acknowledged</span>
                  <span className="stat-row-value">{new Date(detailAlert.acknowledged_at).toLocaleString()}</span>
                </div>
              )}
              {detailAlert.resolved_at && (
                <div>
                  <span className="stat-row-label">Resolved</span>
                  <span className="stat-row-value">{new Date(detailAlert.resolved_at).toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
