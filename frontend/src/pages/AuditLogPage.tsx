    ...
    <motion.div variants={stagger} initial="hidden" animate="visible">
      ...
      <motion.div className="card" variants={item}>
        <div className="card-header">
          <h3><Verified size={14} style={{ marginRight: 8, color: "var(--accent-green)" }} /> Audit Trail</h3>
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {loading ? (
            ...
            <EmptyState icon={Shield} title="No audit entries" description="Audit logs will appear here as actions are performed in the system." />
          ) : (
            <div>
                ...
                >
                  ...
                  <div className="audit-entry-content">
                    <div className="audit-entry-header">
                      ...
                      <span className="badge allow" style={{ fontSize: "10px", padding: "1px 6px" }}>
                        <span className="badge-dot" />
                        HMAC Signed
                      </span>
                    </div>
                    <div className="audit-entry-detail">
                      {entry.user_email && <span>User: <strong>{entry.user_email}</strong></span>}
                      {entry.resource_type && <span>Resource: <strong>{entry.resource_type}</strong></span>}
                      {entry.resource_id && <span>ID: <code className="audit-entry-code">{entry.resource_id}</code></span>}
                    </div>
                    {entry.details && (
                      <details className="audit-details">
                        <summary style={{ fontSize: "11px", color: "var(--stone)", cursor: "pointer" }}>
